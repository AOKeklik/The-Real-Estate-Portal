        ...
        if(empty($errors)){
            try{
                ...
                try{
                    ...
                    $phpMailler->addAddress($mainMessage["agent_email"]);
                    
                    $link=BASE_URL."agent-message/".$message_id;

                    $phpMailler->isHTML(true);
                    $phpMailler->Subject="Customer Message";
                    $phpMailler->Body = "<p>A customer has sent you message. So please login to your account and check that.</p>";
                    ...
                    header("Location: ".BASE_URL."customer-message/".$message_id);
                    exit();
                }catch(Exception $err){
                    $error_message=$err->getMessage();
                }
                    ...
                    <div class="message-item">
                        <div class="message-top">
                            ...
                            <div class="text">
                                <h6>
                                    <?php if(preg_match("/customer/i",$replyMessage["sender"])):?>
                                        <?php if(!is_null($replyMessage["customer_name"])) echo $mainMessage["customer_name"]?> <span class="badge rounded-pill text-bg-primary">Customer</span>
                                    <?php else:?>
                                        <?php echo $mainMessage["agent_name"]?> <span class="badge rounded-pill text-bg-success">Agent</span>
                                    <?php endif?>
                                </h6>
