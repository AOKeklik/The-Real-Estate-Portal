        ...
        if(empty($errors)){
            try{
                ...
                if(!$stmt->execute([$text,$title]))
                    throw new PDOException("An error occurred while updating the yerms of Use text. Please try again.");

                $_SESSION["success"]="Terms of Use text has been successfully updated.";
                header("Location: ".ADMIN_URL."terms-edit");
                exit();
            }catch(PDOException $err){
                $error_message=$err->getMessage();
            }
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form action="" method="post">
                                <div class="row">   
                                    <div class="col-md-3">
                                        <span class="badge badge-success" style="<?php if($terms["status"] == 0) echo "display:none"?>">Yes</span>
                                        <span class="badge badge-danger" style="<?php if($terms["status"] == 1) echo "display:none"?>">No</span>
                                    </div>                                         
                                    <div class="col-md-9">
                                        <div class="wrapper-loader-btn" style="display: inline-block;">
                                            ...
                                            <input 
                                                name="status" 
                                                <?php if($terms["status"] == 1) echo "checked"?> 
                                                data-post-id="<?php echo $terms["id"]?>"
                                                type="checkbox" 
                                                data-toggle="toggle" 
                                ...
                                <div class="form-group mb-3">
                                    <label>Title *</label>
                                    <input type="text" class="form-control" name="title" value="<?php echo $terms["title"]?>">
                                    <?php if(isset($errors["title"])) echo $errors["title"][0]?>
                                </div>
                                <div class="form-group mb-3">
                                    <label>Description *</label>
                                    <textarea name="text" class="form-control editor" cols="30" rows="10"><?php echo $terms["text"]?></textarea>
...
<script>
    $(document).ready(function(){
        $("input[name=status]").change(async function(e){
            ...
            div.addClass("pentling")
            div.removeClass("active")
            await new Promise(resolve=>setTimeout(resolve,1000))
            
            $.ajax({
                url: "<?php echo ADMIN_URL?>page_terms_edit_status_ajax.php",
                type: "POST",
                processData:false,
                contentType:false,
                data: formData,
